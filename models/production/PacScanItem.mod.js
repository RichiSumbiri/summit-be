...
FROM (
	...
	FIND_IN_SET( LEFT(d.ARTICLE_GENERIC,6), LEFT(b.PRODUCT_ITEM_CODE, 6)) > 0 AND a.PO_ITEM = d.PO_ITEM
	...
	WHERE a.BUYER_PO =  :poBuyer AND SUBSTRING_INDEX(SUBSTRING_INDEX(a.ROWID, '|', 4), '|', -1) = 'SOLID'
	UNION ALL 
		SELECT a.ROWID, a.PACKPLAN_ID, e.PACKPLAN_EX_FACTORY, 
	 SUBSTRING_INDEX(SUBSTRING_INDEX(a.ROWID, '|', 4), '|', -1) PACKING_METHODE,
	 h.COUNTRY_NAME, a.ROW_TYPE, a.BOX_ID, a.CTN_START, a.CTN_END, a.PO_INDEX, a.COL_INDEX,
	...
	LEFT JOIN (
			...
			f.SEGMENT, f.ARTICLE, f.COLOR_DESCRIPTION
			FROM order_po_buyer f
			WHERE f.PO_NUMBER = :poBuyer AND f.SEGMENT = 'OTH_E'
	      GROUP BY f.PO_NUMBER, f.ARTICLE_GENERIC, f.PO_ITEM, f.COLOR_CODE, f.SIZE_CODE
	) d ON d.PO_NUMBER = a.BUYER_PO AND a.BUYER_COLOR_CODE = d.COLOR_CODE AND a.SIZE_CODE = d.SIZE_CODE AND 
	FIND_IN_SET( LEFT(d.ARTICLE_GENERIC,6), LEFT(b.PRODUCT_ITEM_CODE, 6)) > 0 AND a.PO_ITEM = d.PO_ITEM
	 LEFT JOIN packing_plan_header e ON e.PACKPLAN_ID = b.PACKPLAN_ID 
	...
	WHERE a.BUYER_PO =  :poBuyer AND SUBSTRING_INDEX(SUBSTRING_INDEX(a.ROWID, '|', 4), '|', -1) = 'ECOM'
	UNION ALL
	SELECT a.ROWID, a.PACKPLAN_ID, e.PACKPLAN_EX_FACTORY, 
	 SUBSTRING_INDEX(SUBSTRING_INDEX(a.ROWID, '|', 4), '|', -1) PACKING_METHODE,
	 h.COUNTRY_NAME, a.ROW_TYPE, a.BOX_ID, a.CTN_START, a.CTN_END, a.PO_INDEX, a.COL_INDEX,
