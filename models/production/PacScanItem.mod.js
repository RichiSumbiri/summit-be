...
export const qryLisPoByrWthOutput = `SELECT 
    a.VENDOR, a.PO_NUMBER, a.EX_FACTORY, a.PO_ITEM, a.SEGMENT, a.ARTICLE_GENERIC, a.ARTICLE, a.COLOR_CODE, a.COLOR_DESCRIPTION, a.UPC_CODE, a.SIZE_CODE, 
    SUM(a.QTY) QTY,
    COALESCE(b.BOOKING_QTY, 0) AS BOOKING_QTY,
    COALESCE(c.SCAN_QTY, 0) AS SCAN_QTY
FROM order_po_buyer a
LEFT JOIN (
    SELECT 
        a.BUYER_PO, a.BUYER_COLOR_CODE, a.PO_ITEM, a.UPC_CODE, a.SIZE_CODE, SUM(a.TTL_QTY_BOX) BOOKING_QTY
    FROM packing_plan_box_row a 
    WHERE a.BUYER_PO = :poNum
    GROUP BY a.BUYER_PO, a.BUYER_COLOR_CODE, a.PO_ITEM, a.UPC_CODE, a.SIZE_CODE
) b 
...
LEFT JOIN (
    SELECT 
        a.BUYER_PO, a.PO_ITEM, a.UPC_CODE, SUM(a.QTY_PER_BOX) SCAN_QTY
    FROM packing_plan_box_row a 
    LEFT JOIN packing_item_scan b ON b.ROWID = a.ROWID
    LEFT JOIN packing_plan_header c ON a.PACKPLAN_ID = c.PACKPLAN_ID 
    WHERE a.BUYER_PO = :poNum
    GROUP BY a.BUYER_PO, a.PO_ITEM, a.UPC_CODE
) c 
ON c.BUYER_PO = a.PO_NUMBER 
AND c.UPC_CODE = a.UPC_CODE
AND c.PO_ITEM = a.PO_ITEM

WHERE a.PO_NUMBER = :poNum
GROUP BY a.VENDOR, a.PO_NUMBER, a.EX_FACTORY, a.PO_ITEM, a.SEGMENT, a.ARTICLE_GENERIC, a.COLOR_CODE, a.UPC_CODE, a.SIZE_CODE
ORDER BY  a.VENDOR, a.PO_NUMBER, a.EX_FACTORY, a.ARTICLE,  a.COLOR_CODE, a.SEGMENT
`;
