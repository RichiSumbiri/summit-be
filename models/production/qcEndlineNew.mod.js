...
SELECT 
    ...
    M.ORDER_QTY,
    M.TRANSFER_QTY,
	M.ORDER_QTY-IFNULL(M.TRANSFER_QTY,0) BAL_TRANSFER,
    M.RETURN_STATUS,
    M.COUNT_SPLIT
FROM (
	SELECT a.SCH_ID,
	...
	FROM (
		SELECT a.SCH_ID, a.SCHD_ID, DATE(a.SEWING_SCAN_TIME) SCAN_DATE, d.BUYER_CODE, d.ORDER_COLOR,
		 a.BARCODE_SERIAL, f.BARCODE_SERIAL BARCODE_TRANSFER, d.ORDER_SIZE, d.BUYER_PO, d.MO_NO, d.ORDER_QTY,	
		 CASE WHEN f.BARCODE_SERIAL IS NOT NULL THEN IFNULL(f.TRANSFER_QTY,d.ORDER_QTY) ELSE 0 END AS TRANSFER_QTY,
		 f.COUNT_SPLIT
		FROM scan_sewing_in a 
		...
		LEFT JOIN (
			...
			FROM scan_sewing_out a 
			LEFT JOIN scan_sewing_qr_split b ON a.BARCODE_MAIN = b.BARCODE_MAIN
	...
	LEFT JOIN order_po_listing h ON h.ORDER_PO_ID = a.BUYER_PO
	WHERE a.ORDER_QTY <> a.TRANSFER_QTY AND a.SCAN_DATE <= :endDate
) M`;
