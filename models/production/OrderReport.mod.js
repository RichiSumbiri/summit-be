export const QueryGetOrderRepMo = `SELECT 
...
IFNULL(FB.SEWING_IN, 0) SEWING_IN,
 ...
 IFNULL(FB.PACKING_OUT, 0) PACKING_OUT,
 IFNULL(FB.PACKING_SLIP_QTY, 0) PACKING_SLIP_QTY,
 ...
 IFNULL((FA.ORDER_QTY-FB.PACKING_OUT), 0) VAR_ORDER_PACKING_OUT,
 IFNULL((FA.ORDER_QTY-FB.PACKING_SLIP_QTY), 0) VAR_ORDER_PACKING_SLIP_QTY,
 ...
 ROUND((IFNULL(FB.PACKING_OUT / FA.ORDER_QTY, 0) * 100), 2)  AS TTL_PACKING_OUT_PERCENTAGE,
 ROUND((IFNULL(FB.PACKING_SLIP_QTY / FA.ORDER_QTY, 0) * 100), 2)  AS TTL_PACKING_SLIP_QTY_PERCENTAGE,
 ...
 IF(IFNULL(FB.PACKING_OUT / FA.ORDER_QTY, 0) * 100 >= 100, 'Complete', 'Open') AS PACKING_OUT_STATUS,
  IF(IFNULL(FB.PACKING_SLIP_QTY / FA.ORDER_QTY, 0) * 100 >= 100, 'Complete', 'Open') AS PACKING_SLIP_STATUS
FROM ( 
...
LEFT JOIN (
	...
	SUM(C.PACKING_OUT) AS PACKING_OUT,
	SUM(D.PACKING_SLIP_QTY) AS PACKING_SLIP_QTY
	FROM (
		...
		LEFT JOIN (
			...
			WHERE  a.MOID IN  (
				SELECT DISTINCT a.MO_NO FROM order_po_listing_size a 
					WHERE a.PRODUCTION_MONTH IN (:listMonth) 
					GROUP BY a.MO_NO
				) GROUP BY a.MOID, a.FGColorID
		)C ON A.MO_NO = C.MO_NO
		LEFT JOIN (
			SELECT N.ORDER_PO_ID, N.MO_NO, SUM(N.PACKING_SLIP_QTY) PACKING_SLIP_QTY
			FROM (
				SELECT a.OrderPOID ORDER_PO_ID, b.MO_NO, a.PKSQty PACKING_SLIP_QTY
				FROM FX_CustomerShipmentDetail a
				LEFT JOIN  (
						SELECT DISTINCT  a.ORDER_PO_ID, a.MO_NO
						FROM order_po_listing_size a 
							WHERE a.PRODUCTION_MONTH IN (:listMonth) 
							GROUP BY a.ORDER_PO_ID
						) b ON b.ORDER_PO_ID = a.OrderPOID
				WHERE  a.OrderPOID IN  (
						SELECT DISTINCT a.ORDER_PO_ID FROM order_po_listing_size a 
						WHERE a.PRODUCTION_MONTH IN (:listMonth) 
						GROUP BY a.ORDER_PO_ID
					)
			) N GROUP BY  N.MO_NO		
		) D ON A.MO_NO = D.MO_NO
...
FA.ID_CAPACITY, FA.PRODUCTION_MONTH, DATE_FORMAT(FA.PLAN_EXFACTORY_DATE, '%M/%Y') EXPORT_MONTH,  FA.PROD_MONTH, FA.ACTUAL_PROD_MONTH, FA.MANUFACTURING_SITE, FA.CUSTOMER_NAME,FA.CUSTOMER_DIVISION, FA.CUSTOMER_PROGRAM,
FA.CUSTOMER_SEASON, FA.ORDER_NO, FA.ORDER_REFERENCE_PO_NO, FA.PRODUCT_ITEM_CODE, FA.PRODUCT_ITEM_DESCRIPTION, FA.ORDER_STYLE_DESCRIPTION,
FA.ITEM_COLOR_CODE, FA.ITEM_COLOR_NAME, FA.MO_NO, FA.TARGET_PCD, FA.PLAN_EXFACTORY_DATE, FA.FINAL_DELIVERY_DATE,
...
FC.PACKING_OUT TTL_PACKING_OUT,
FD.PACKING_SLIP_QTY, 
...
IFNULL((FA.ORDER_QTY-FC.PACKING_OUT), 0) VAR_ORDER_PACKING_OUT,
IFNULL((FA.ORDER_QTY-FD.PACKING_SLIP_QTY), 0) VAR_ORDER_PACKING_SLIP_QTY,
ROUND((IFNULL(FA.SCH_QTY / FA.ORDER_QTY, 0) * 100), 2) AS TTL_SCHEDULE_PERCENTAGE,
...
ROUND((IFNULL(FC.PACKING_OUT / FA.ORDER_QTY, 0) * 100), 2) AS TTL_PACKING_OUT_PERCENTAGE,
ROUND((IFNULL(FD.PACKING_SLIP_QTY / FA.ORDER_QTY, 0) * 100), 2) AS TTL_PACKING_SLIP_QTY_PERCENTAGE,
IF(IFNULL(FA.SCH_QTY / FA.ORDER_QTY, 0) * 100 >= 100, 'Complete', 'Open') AS SCHEDULE_STATUS,
...
IF(IFNULL(FC.PACKING_OUT / FA.ORDER_QTY, 0) * 100 >= 100, 'Complete', 'Open') AS PACKING_OUT_STATUS, 
IF(IFNULL(FD.PACKING_SLIP_QTY / FA.ORDER_QTY, 0) * 100 >= 100, 'Complete', 'Open') AS PACKING_SLIP_STATUS, 
...
FROM (
	...
	a.CUSTOMER_SEASON, a.ORDER_NO, a.ORDER_REFERENCE_PO_NO, a.PRODUCT_ITEM_CODE, a.PRODUCT_ITEM_DESCRIPTION, a.ORDER_STYLE_DESCRIPTION,
	a.ITEM_COLOR_CODE, a.ITEM_COLOR_NAME, a.MO_NO, a.TARGET_PCD, 
	IFNULL( a.NEW_PLAN_EXFACTORY_DATE,a.PLAN_EXFACTORY_DATE) PLAN_EXFACTORY_DATE, 
--	a.PLAN_EXFACTORY_DATE,
	a.FINAL_DELIVERY_DATE,
...
LEFT JOIN (
	SELECT 
	n.ID_CAPACITY, 
	n.MANUFACTURING_SITE, n.PRODUCTION_MONTH, n.CUSTOMER_NAME,  n.PRODUCT_ITEM_CODE, n.ORDER_PO_ID, n.ORDER_NO, n.MO_NO, 
	n.ITEM_COLOR_CODE, n.ITEM_COLOR_NAME, n.PLAN_EXFACTORY_DATE,
	SUM(n.PACKING_OUT) PACKING_OUT, SUM(n.PACKING_OUT_REJECT) PACKING_OUT_REJECT
	FROM (
		SELECT b.ID_CAPACITY, a.OrderPOID, a.MOID MO_NO, b.MANUFACTURING_SITE, b.PRODUCTION_MONTH, b.CUSTOMER_NAME,  
		...
		LEFT JOIN  (
			SELECT DISTINCT 		REPLACE(CONCAT(CAST(a.MANUFACTURING_SITE AS CHAR),".",
				...
				CAST(a.ORDER_NO AS CHAR),".",
				CAST(a.PRODUCT_ITEM_CODE AS CHAR),".",
				CAST(a.ITEM_COLOR_CODE AS CHAR),".",
				CAST(a.PLAN_EXFACTORY_DATE AS CHAR),".",
				CAST(a.PRODUCTION_MONTH AS CHAR)
				),'-','') ID_CAPACITY, a.ORDER_PO_ID,
				a.MANUFACTURING_SITE, a.PRODUCTION_MONTH, a.CUSTOMER_NAME,  a.PRODUCT_ITEM_CODE,  a.ORDER_NO, a.MO_NO, 
				a.ITEM_COLOR_CODE, a.ITEM_COLOR_NAME, a.PLAN_EXFACTORY_DATE, 
				SUM(a.ORDER_QTY) ORDER_QTY
			FROM order_po_listing_size a 
			WHERE a.PRODUCTION_MONTH IN (:listMonth) 
			GROUP BY a.ORDER_PO_ID
		...
		GROUP BY a.OrderPOID
	)n GROUP BY n.ID_CAPACITY
) FC ON 
		FA.PRODUCTION_MONTH = FC.PRODUCTION_MONTH AND 
		FA.MANUFACTURING_SITE = FC.MANUFACTURING_SITE AND
		FA.ORDER_NO = FC.ORDER_NO AND
		FA.ITEM_COLOR_CODE = FC.ITEM_COLOR_CODE AND
		FA.PLAN_EXFACTORY_DATE = FC.PLAN_EXFACTORY_DATE
LEFT JOIN (
		SELECT a.OrderPOID ORDER_PO_ID, b.ID_CAPACITY, IFNULL(SUM(a.PKSQty),0) PACKING_SLIP_QTY, 
		b.MANUFACTURING_SITE, b.PRODUCTION_MONTH, b.CUSTOMER_NAME,  
		a.ProductItemCode PRODUCT_ITEM_CODE,  b.ORDER_NO, b.ITEM_COLOR_CODE, b.ITEM_COLOR_NAME, b.PLAN_EXFACTORY_DATE
		FROM FX_CustomerShipmentDetail a
		LEFT JOIN  (
					SELECT DISTINCT 		REPLACE(CONCAT(CAST(a.MANUFACTURING_SITE AS CHAR),".",
						CAST(a.CUSTOMER_NAME AS CHAR),".",
		...
		WHERE  a.OrderPOID IN  (
			...
			WHERE a.PRODUCTION_MONTH IN (:listMonth) 
			GROUP BY a.ORDER_PO_ID
		) GROUP BY b.ID_CAPACITY
) FD ON 
		FA.PRODUCTION_MONTH = FD.PRODUCTION_MONTH AND 
		FA.MANUFACTURING_SITE = FD.MANUFACTURING_SITE AND
		FA.ORDER_NO = FD.ORDER_NO AND
		FA.ITEM_COLOR_CODE = FD.ITEM_COLOR_CODE AND
		FA.PLAN_EXFACTORY_DATE = FD.PLAN_EXFACTORY_DATE
