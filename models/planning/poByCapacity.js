...
FROM viewcapacity a 
LEFT JOIN (
			SELECT b.MANUFACTURING_SITE, b.CUSTOMER_NAME, a.SCH_CAPACITY_ID, SUM(a.SCH_QTY) SCH_QTY,  COUNT(a.SCH_ID_SITELINE) COUNT_SITELINE, MIN(a.SCH_START_PROD) SCH_START_PROD, MAX(a.SCH_FINISH_PROD) SCH_FINISH_PROD
			FROM  viewcapacity b  
...
export const QueryGetWeekSchPo = `SELECT b.MANUFACTURING_SITE, a.SCH_SITE, b.CUSTOMER_NAME, a.SCH_CAPACITY_ID, a.SCH_ID_SITELINE, c.LINE_NAME, a.SCH_ORDER,
b.ORDER_REFERENCE_PO_NO, b.ITEM_COLOR_NAME, a.SCH_START_PROD, a.SCH_FINISH_PROD,
...
export const QueryListlineByStylePo = `SELECT
...
FROM
	viewcapacity b
LEFT JOIN weekly_prod_schedule a ON
	a.SCH_CAPACITY_ID = b.ID_CAPACITY
	AND b.MANUFACTURING_SITE = :siteName
LEFT JOIN item_siteline c ON
	a.SCH_ID_SITELINE = c.ID_SITELINE
WHERE
	b.PROD_MONTH BETWEEN :startMonth AND :endMonth
	AND b.MANUFACTURING_SITE = :siteName
	AND a.SCH_ID_SITELINE IS NOT NULL
GROUP BY
	b.MANUFACTURING_SITE,
	b.CUSTOMER_NAME,
	a.SCH_ID_SITELINE,
...
export const getSizeAlocSize = `SELECT n.SCH_ID, b.SCH_CAPACITY_ID,  b.SCH_ID_SITELINE, n.SIZE_CODE,  sum(n.SCH_SIZE_QTY) SCH_SIZE_QTY
FROM view_weekly_sch_size n
LEFT JOIN weekly_prod_schedule b ON b.SCH_ID = n.SCH_ID
WHERE n.ID_CAP_SIZE = CONCAT(SUBSTRING_INDEX( :capId,'.',5),'.', SUBSTRING_INDEX( :capId,'.',-1))
AND n.SCH_SIZE_QTY <> 0
GROUP BY n.SCH_ID, b.SCH_ID_SITELINE, n.SIZE_CODE `;
