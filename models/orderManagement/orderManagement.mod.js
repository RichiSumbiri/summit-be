...
export const queryRecapToPOMatrixDelivery = `
SELECT
	pmd.MANUFACTURING_SITE SITE_CODE,
	pmd.PRODUCTION_MONTH PROD_MONTH,
	pmd.CUSTOMER_NAME BUYER_CODE,
	pmd.ORDER_NO,
	pmd.ORDER_REFERENCE_PO_NO ORDER_REF_NO,
	pmd.ORDER_STYLE_DESCRIPTION ORDER_PO_STYLE_REF,
	pmd.ITEM_COLOR_CODE COLOR_CODE,
	pmd.ITEM_COLOR_NAME COLOR_NAME,
	opl.PACKING_METHOD PACKING_METHOD,
	pmd.PLAN_EXFACTORY_DATE EX_FACTORY,
	pmd.SIZE_CODE  SIZE_CODE,
	SUM(pmd.ORDER_QTY) TOTAL_QTY,
	pmd.CREATE_DATE PDM_ADD_DATE,
	pmd.UPDATE_DATE PDM_MOD_DATE,
	pmd.UPDATE_BY PDM_MOD_ID,
	pmd.CREATE_BY PDM_ADD_ID
FROM
	order_po_listing_size pmd 
LEFT JOIN order_po_listing opl ON opl.ORDER_NO = pmd.ORDER_NO AND opl.ORDER_PO_ID = pmd.ORDER_PO_ID 
WHERE pmd.ORDER_NO = :orderID
GROUP BY pmd.ORDER_NO , pmd.ITEM_COLOR_CODE , pmd.PRODUCTION_MONTH , pmd.SIZE_CODE , pmd.PLAN_EXFACTORY_DATE 

`;


export const queryRecapPOListingDetail = `
SELECT
  opl.MANUFACTURING_COMPANY,
  opl.ORDER_PLACEMENT_COMPANY,
  cd.CTC_NAME CUSTOMER_NAME,
  cpd.CTPROD_DIVISION_NAME AS CUSTOMER_DIVISION_NAME,
  cps.CTPROD_SESION_NAME CUSTOMER_SEASON_NAME,
  cpn.CTPROG_ID CUSTOMER_PROGRAM_NAME,
  cbp.CTBUYPLAN_NAME CUSTOMER_BUYPLAN_NAME,
  po.PRJ_ID PROJECTION_ORDER_ID,
  po.PRJ_CODE PROJECTION_ORDER_CODE,
  opl.ORDER_TYPE_CODE,
  opl.ORDER_NO,
  opl.ORDER_REFERENCE_PO_NO,
  opl.ORDER_STYLE_DESCRIPTION,
  ...
  opl.PRODUCT_ITEM_DESCRIPTION,
  oph.PRODUCT_ID,
  pi2.PRODUCT_TYPE_CODE,
  pi2.PRODUCT_CAT_CODE,
  opl.ITEM_COLOR_CODE,
  opl.ITEM_COLOR_NAME,
  opl.ORDER_QTY,
  opl.MO_QTY,
  opl.SHIPMENT_PO_QTY,
  opl.ORDER_UOM,
  (opl.MO_QTY / opl.ORDER_QTY ) PLAN_MO_PERCENTAGE,
  (opl.SHIPMENT_PO_QTY - opl.ORDER_QTY ) PLAN_SHIPMENT_VARIANCE,
  (opl.SHIPMENT_PO_QTY / opl.ORDER_QTY ) PLAN_SHIPMENT_PERCENTAGE,
  opl.SHIPPED_QTY,
  (opl.SHIPPED_QTY / opl.ORDER_QTY ) ORDER_TO_SHIPPED_PERCENTAGE,
  opl.DELIVERY_TERM,
  opl.PRICE_TYPE,
  opl.UNIT_PRICE,
  opl.MO_COST,
  (opl.ORDER_QTY * opl.UNIT_PRICE ) TOTAL_ORDER_COST,
  (opl.MO_QTY * opl.MO_COST  ) TOTAL_MO_COST,
  opl.CURRENCY_CODE,
  opl.DELIVERY_LOCATION_ID,
  cdl.CTLOC_NAME DELIVERY_LOCATION_NAME,
  opl.COUNTRY,
  opl.PACKING_METHOD,
  opl.DELIVERY_MODE_CODE,
  opl.PO_CREATED_DATE,
  opl.PO_CONFIRMED_DATE,
  oph.PLAN_CUT_DATE TARGET_PCD,
  oph.PLAN_SEW_DATE TARGET_PSD,
  opl.ORIGINAL_DELIVERY_DATE,
  opl.FINAL_DELIVERY_DATE,
  opl.PLAN_EXFACTORY_DATE,
  opl.PO_EXPIRY_DATE,
  ...
  opl.PRODUCT_ITEM_CODE,
  opl.PRODUCT_ITEM_DESCRIPTION,
  (pi2.PRODUCT_DESCRIPTION) ORDER_PRODUCT_ITEM_TYPE_CATEGORY,
  NULL AS SERVICE_ATTRIBUTE_BONDING_CODE,
  NULL AS SERVICE_ATTRIBUTE_FUSING_CODE,
  NULL AS SERVICE_ATTRIBUTE_HEAT_TRANSFER_LABEL_CODE,
  NULL AS SERVICE_ATTRIBUTE_LAMINATING_CODE,
  NULL AS SERVICE_ATTRIBUTE_MOULDING_CODE,
  NULL AS SERVICE_ATTRIBUTE_PRINTING_CODE,
  NULL AS SERVICE_ATTRIBUTE_STRAP_ASSEMBLY_CODE,
  NULL AS SERVICE_ATTRIBUTE_EMBROIDERY_CODE,
  NULL AS SERVICE_ATTRIBUTE_DYEING_CODE
FROM order_po_listing opl
LEFT JOIN customer_detail cd ON cd.CTC_ID = opl.CUSTOMER_ID 
LEFT JOIN customer_product_division cpd ON cpd.CTPROD_DIVISION_ID = opl.CUSTOMER_DIVISION_ID 
LEFT JOIN customer_product_season cps ON cps.CTPROD_SESION_ID = opl.CUSTOMER_SEASON_ID 
LEFT JOIN customer_program_name cpn ON cpn.CTPROG_ID = opl.CUSTOMER_PROGRAM_ID 
LEFT JOIN customer_buy_plan cbp ON cbp.CTBUYPLAN_ID = opl.CUSTOMER_BUYPLAN_ID 
LEFT JOIN customer_delivery_loc cdl ON cdl.CTLOC_ID = opl.DELIVERY_LOCATION_ID 
LEFT JOIN projection_order po ON po.PRJ_ID = opl.PROJECTION_ORDER_ID 
LEFT JOIN order_po_header oph ON oph.ORDER_ID = opl.ORDER_NO 
LEFT JOIN product_item pi2 ON pi2.PRODUCT_ID = oph.PRODUCT_ID 
WHERE 
  opl.SUMMIT_FLAG ='1'
...
export const queryRecapPOListingSize = `
SELECT
	opls.MANUFACTURING_COMPANY,
	opls.ORDER_PLACEMENT_COMPANY,
	opls.ORDER_NO,
	oph.ORDER_REFERENCE_PO_NO,
	oph.ORDER_STYLE_DESCRIPTION,
	opl.ORDER_PO_ID,
	...
	oph.CUSTOMER_ID,
	cd.CTC_NAME CUSTOMER_NAME,
	cpd.CTPROD_DIVISION_NAME CUSTOMER_DIVISION_NAME,
	cps.CTPROD_SESION_NAME CUSTOMER_SEASON_NAME,
	cpn.CTPROG_NAME CUSTOMER_PROGRAM_NAME,
	cbp.CTBUYPLAN_NAME CUSTOMER_BUYPLAN_NAME,
	oph.ITEM_ID PRODUCT_ITEM_ID,
	mii.ITEM_CODE PRODUCT_ITEM_CODE,
	mii.ITEM_DESCRIPTION PRODUCT_ITEM_DESCRIPTION,
	oph.PRODUCT_ID,
	pi2.PRODUCT_TYPE_CODE,
	pi2.PRODUCT_CAT_CODE,
	opl.MO_AVAILABILITY,
  ...
  opl.PRODUCTION_MONTH ,
	opl.MANUFACTURING_SITE  
FROM order_po_listing_size opls 
LEFT JOIN order_po_listing opl ON opl.ORDER_PO_ID = opls.ORDER_PO_ID AND opl.ORDER_NO = opls.ORDER_NO 
LEFT JOIN order_po_header oph ON oph.ORDER_ID = opl.ORDER_NO 
LEFT JOIN customer_detail cd ON cd.CTC_ID = oph.CUSTOMER_ID 
LEFT JOIN customer_product_division cpd ON cpd.CTPROD_DIVISION_ID = oph.CUSTOMER_DIVISION_ID 
LEFT JOIN customer_product_season cps ON cps.CTPROD_SESION_ID = oph.CUSTOMER_SEASON_ID 
