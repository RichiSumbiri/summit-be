...
export const punchAttdLog = async (req, res) => {
  try {
    ...
    const stringStart = moment(dates.start).format("YYYY-MM-DD");
    const stringEnd = moment(dates.end).format("YYYY-MM-DD");
    const arrDate = `'${stringStart}','${stringEnd}'`;
    // const arrDate = dates.map((st) => `'${st}'`).join(", ");
        ...
        if (logs.log_status === "IN") {
          ...
          if (findSch && !findSch.id) {
            ...
            const dataAbsen = {
              ...
              ket_in: checkLate ? 'LATE' : null //harus cek lembur
            };
