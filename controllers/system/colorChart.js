...
export const getColors = async (req, res) => {
  try {
    const { SEARCH_TEXT, page, limit, is_filter } = req.query;

    const include = SEARCH_TEXT || is_filter
      ? []
      : [
          {
            model: Users,
    ...
    const attributes = is_filter
      ? ["COLOR_ID", "COLOR_CODE", "COLOR_DESCRIPTION"]
      : undefined;
      ...
      const existing = await colorItemCategory.findOne({
        where: {
          ...
          ITEM_CATEGORY_ID: Number(
            colorData?.COLOR_ITEM_CATEGORY?.ITEM_CATEGORY_ID
          ),
        },
      });
      if (existing) {
        return errorResponse(
          res,
          null,
          "Color already exist in this ccategory",
          400
        );
      }
    } else {
