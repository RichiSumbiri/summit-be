...
import eventTemplateLine, {
  QueryGetDiaryEventListingReport,
} from "../../../models/tna/eventTemplateLine.mod.js";
...
export const getEventListingMaster = async (req, res) => {
  try {
    const {
      CUSTOMER_ORDER_TYPE,
...
export const getEventListingDiary = async (req, res) => {
  try {
    const {
      CUSTOMER_ORDER_TYPE,
      CUSTOMER_ORDER_TYPE_FROM,
    ...
    // event_framework
    if (ORDER_ID) {
      whereClauses.push(`e.ORDER_ID = '${ORDER_ID}'`);
    }

    // order_po_header
    if (CUSTOMER_ID) {
      whereClauses.push(`d.CUSTOMER_ID = '${CUSTOMER_ID}'`);
    }
    if (CUSTOMER_SEASON_ID) {
      whereClauses.push(`d.CUSTOMER_SEASON_ID = '${CUSTOMER_SEASON_ID}'`);
    }
    if (CUSTOMER_DIVISION_ID) {
      whereClauses.push(`d.CUSTOMER_DIVISION_ID = '${CUSTOMER_DIVISION_ID}'`);
    }
    if (
      CUSTOMER_ORDER_TYPE_FROM &&
      CUSTOMER_ORDER_TYPE_TO &&
      ...
      const from = `${CUSTOMER_ORDER_TYPE_FROM} 00:00:00`;
      const to = `${CUSTOMER_ORDER_TYPE_TO} 23:59:59`;

      if (CUSTOMER_ORDER_TYPE.toLowerCase() === "all") {
        whereClauses.push(`(
          d.PLAN_CUT_DATE BETWEEN '${from}' AND '${to}' OR
          d.PLAN_SEW_DATE BETWEEN '${from}' AND '${to}' OR
          d.PLAN_FIN_DATE BETWEEN '${from}' AND '${to}' OR
          d.PLAN_PP_MEETING BETWEEN '${from}' AND '${to}' OR
          d.ORDER_CONFIRMED_DATE BETWEEN '${from}' AND '${to}'
        )`);
      } else {
        whereClauses.push(
          `d.${CUSTOMER_ORDER_TYPE} BETWEEN '${from}' AND '${to}'`
        );
      }
    }
    ...
    // event_master
    if (EVENT_TYPE_ID) {
      whereClauses.push(`f.EVENT_TYPE_ID = '${EVENT_TYPE_ID}'`);
    }
    if (EVENT_GROUP_ID) {
      whereClauses.push(`f.EVENT_GROUP_ID = '${EVENT_GROUP_ID}'`);
    }

    // event_diary_header
    if (COMMITMENT_DATE_FROM && COMMITMENT_DATE_TO) {
      whereClauses.push(
        `b.COMMITMENT_DATE BETWEEN '${COMMITMENT_DATE_FROM}' AND '${COMMITMENT_DATE_TO}'`
      );
    }

    // event_diary_status
    const statusConditions = [];
    if (IS_DIARY_COMPLETE) {
      statusConditions.push(`a.COMPLETED_STATUS = 'Complete'`);
    }
    if (IS_DIARY_OVERDUE) {
      statusConditions.push(
        `a.COMPLETED_STATUS != 'Complete' AND b.COMMITMENT_DATE < CURDATE()`
      );
    }
    if (IS_DIARY_IN_PROGRESS) {
      statusConditions.push(
        `a.COMPLETED_STATUS != 'Complete' AND b.COMMITMENT_DATE >= CURDATE()`
      );
    }
    ...
    if (PRODUCTION_MONTH) {
      ...
      whereClauses.push(`c.PRODUCTION_MONTH = '${monthName}/${year}'`);
    }
    ...
    // post-fetch filters (status + disparity)
    diaryMasterData = diaryMasterData.filter((row) => {
      let statusOk = true;
      if (IS_COMPLETE || IS_IN_PROGRESS || IS_OVERDUE) {
        statusOk = false;
      ...
      let disparityOk = true;
      if (DISPARITY) {
        const commitmentDate = row.COMMITMENT_DATE
          ? new Date(row.COMMITMENT_DATE)
          : null;
        const targetDate = row.TARGET_DATE ? new Date(row.TARGET_DATE) : null;

        if (!commitmentDate || !targetDate) disparityOk = false;

        if (DISPARITY === "commitment_late" && !(commitmentDate > targetDate))
          disparityOk = false;
        if (DISPARITY === "target_late" && !(targetDate > commitmentDate))
          disparityOk = false;
      }
